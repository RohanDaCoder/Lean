const { SlashCommandBuilder, ActionRowBuilder, StringSelectMenuBuilder, StringSelectMenuOptionBuilder, EmbedBuilder, ButtonBuilder, ButtonStyle } = require('discord.js');
...
module.exports = {
	...
	async run({ interaction, client }) {
		...
		try {
			...
			const filter = i => i.customId === 'unban-select' && i.user.id === interaction.user.id;
			...
			collector.on('collect', async i => {
				...
				const buttonFilter = btn => ['confirm-unban', 'cancel-unban'].includes(btn.customId) && btn.user.id === interaction.user.id;
					...
					if (btn.customId === 'confirm-unban') {
						try {
							...
							const unbanEmbed = new EmbedBuilder()
								...
								.addFields(
									{ name: 'Unbanned User', value: selectedBan.user.tag, inline: true },
							...
							await btn.update({ content: null, embeds: [unbanEmbed], components: [] });
