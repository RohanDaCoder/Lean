...
module.exports = {
  ...
  async run({ interaction, client }) {
    try {
      ...
      if (nameOption) {
        results = items.filter(item => item.description.toLowerCase().includes(nameOption.toLowerCase()));
        if (results.length === 0) {
          return interaction.editReply(`${config.emojis.no} No item found with name containing "${nameOption}"`);
        }
      } else {
        return interaction.editReply(`${config.emojis.no} Please provide a name to search.`);
      }
      ...
      const row = new ActionRowBuilder()
        .addComponents(
          new ButtonBuilder().setCustomId('previous').setLabel('Previous ◀️').setStyle(ButtonStyle.Secondary),
      ...
      collector.on('collect', async i => {
        try {
          ...
          await interaction.editReply({ embeds: [getItemEmbed(results[index])], components: [row] });
        } catch (error) {
      ...
      await interaction.editReply({ content: `${config.emojis.no} An error occurred while trying to fetch the data.`, ephemeral: true });
    }
  },
};