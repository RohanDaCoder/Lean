...
class GuessTheNumber extends events {
	constructor(options = {}) {
		super();
      ...
      isNaN(options.upperBound)
		...
		if (!options.embed.description) {options.embed.description = `Guess a number between **${options.lowerBound}** and **${options.upperBound}.**\nYou Have **${options.maxAttempts}** Attempts.`;}
		if (!options.winMessage) {options.winMessage = 'You guessed the number! The number was **{number}**. You guessed the number in **{attempts}** attempts.';}
		if (!options.loseMessage) {options.loseMessage = 'You lost! The number was **{number}**.';}
...
module.exports = {
	data: new SlashCommandBuilder()
		...
		.setDescription('Play a guess the number game!'),
	...
	async run({ client, interaction }) {
		try {
			...
			console.error('Error starting Guess The Number game:', error);
			await interaction.reply(
				`${client.config.emojis.no} Failed to start Guess The Number game.`,
			);
		}
	},
};
