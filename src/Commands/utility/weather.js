  ...
  async run({ interaction }) {
    ...
    try {
      ...
      const response = await axios.get(
        `https://api.popcat.xyz/weather?q=${encodeURIComponent(location)}`,
      );
      ...
      const embed = new EmbedBuilder()
        ...
        .setDescription(
          `Current weather and forecast for ${weatherData.location.name}`,
        )
        ...
        .addFields(
          ...
          {
            name: "Wind",
            value: weatherData.current.winddisplay,
            inline: true,
          },
          {
            name: "Observation Time",
            value: weatherData.current.observationtime,
            inline: true,
          },
        )
        .addFields(
