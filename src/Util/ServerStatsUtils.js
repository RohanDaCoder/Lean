...
async function setupServerStats({ interaction }) {
  try {
    ...
    const guildConfigDBPath = path.join(__dirname, `../Database/Guilds/${guildId}.json`);
    ...
    let guildConfig = {
      ...
      totalHumanMembersChannel: await guildConfigDB.get("totalHumanMembersChannel"),
    ...
    const allChannelsExist = guildConfig.serverStatsCategory &&
    ...
    if (allChannelsExist) {
      ...
      const categoryExists = await channelExists(interaction.guild, guildConfig.serverStatsCategory);
      const totalMembersChannelExists = await channelExists(interaction.guild, guildConfig.totalMembersChannel);
      const totalHumanMembersChannelExists = await channelExists(interaction.guild, guildConfig.totalHumanMembersChannel);
      const totalBotsChannelExists = await channelExists(interaction.guild, guildConfig.totalBotsChannel);

      if (categoryExists && totalMembersChannelExists && totalHumanMembersChannelExists && totalBotsChannelExists) {
    ...
    const categoryName = interaction.options.getString("category-name") || "📊Server Stats📊";
    const totalMembersChannelName = interaction.options.getString("total-members-channel-name") || "Total Members";
    const totalHumanMembersChannelName = interaction.options.getString("total-human-members-channel-name") || "Total Human Members";
    const totalBotsChannelName = interaction.options.getString("total-bots-channel-name") || "Total Bots";
    ...
    if (!botMember.permissions.has([
      ...
      return await interaction.followUp({
        content: "I do not have the necessary permissions to manage channels and roles.",
        ephemeral: true,
      });
    }
    ...
    await guildConfigDB.set("totalHumanMembersChannel", totalHumanMembersChannel.id);
...
async function updateServerStats(guild, guildConfig) {
  try {
    ...
    const totalHumanMembersCount = await guild.members.fetch().then(members => members.filter(member => !member.user.bot).size);
    const totalBotsCount = await guild.members.fetch().then(members => members.filter(member => member.user.bot).size);

    await updateChannelName(guild, guildConfig.totalMembersChannel, `Total Members: ${totalMembersCount}`);
    await updateChannelName(guild, guildConfig.totalHumanMembersChannel, `Total Human Members: ${totalHumanMembersCount}`);
    await updateChannelName(guild, guildConfig.totalBotsChannel, `Total Bots: ${totalBotsCount}`);
  } catch (error) {
    console.error("Error updating server stats:", error);
  }
}

async function createOrUpdateChannel(interaction, name, count, existingChannelId, parent) {
