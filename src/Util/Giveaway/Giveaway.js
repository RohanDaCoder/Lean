...
class Giveaway extends EventEmitter {
  ...
  get reaction() {
    if (!this.options.reaction && this.message) {
      const emoji = Discord.resolvePartialEmoji(
        this.manager.options.default.reaction,
      );
  ...
  ensureEndTimeout() {
    ...
    if (
      this.remainingTime >
      (this.manager.options.forceUpdateEvery || DEFAULT_CHECK_INTERVAL)
    )
      return;
  ...
  async fetchAllEntrants() {
    return new Promise(async (resolve, reject) => {
      ...
      while (userCollection.size % 100 === 0) {
        const newUsers = await reaction.users.fetch({
          after: userCollection.lastKey(),
        });
  ...
  async checkWinnerEntry(user) {
    ...
    const member = await this.message?.guild.members
      .fetch(user.id)
      .catch(() => {});
  ...
  async roll(winnerCount = this.winnerCount) {
    ...
    if (
      new Discord.IntentsBitField(this.client.options.intents).has(
      ...
      if (this.client.shard && !guild.shard) {
        guild =
          (await this.client.guilds.fetch(guild.id).catch(() => {})) ?? guild;
        // "Update" the message instance too, if possible.
        this.message =
          (await this.fetchMessage().catch(() => {})) ?? this.message;
      }
    ...
    return await Promise.all(
      winners.map(
        async (user) => await guild.members.fetch(user.id).catch(() => {}),
      ),
    );
  }
  ...
  edit(options = {}) {
    return new Promise(async (resolve, reject) => {
      if (this.ended)
        return reject(
          "Giveaway with message Id " + this.messageId + " is already ended.",
        );
      this.message ??= await this.fetchMessage().catch(() => {});
      if (!this.message)
        return reject(
          "Unable to fetch message with Id " + this.messageId + ".",
        );
      ...
      if (
        ...
        this.options.lastChance = deepmerge(
          this.options.lastChance || {},
          options.newLastChance,
        );
      }
   ...
   */
    ...
    return new Promise(async (resolve, reject) => {
      ...
      if (
        this.endAt < this.client.readyTimestamp ||
        this.isDrop ||
        this.options.pauseOptions?.isPaused
      ) {
        this.endAt = Date.now();
      }
      if (this.options.pauseOptions?.isPaused)
      ...
      if (winners.length > 0) {
        ...
        const winMessage = this.fillInString(
          this.messages.winMessage.content || this.messages.winMessage,
        );
        const message = winMessage?.replace("{winners}", formattedWinners);
        const components = this.fillInComponents(
          this.messages.winMessage.components,
        );

        if (message?.length > 2000) {
          const firstContentPart = winMessage.slice(
            0,
            winMessage.indexOf("{winners}"),
          );
          ...
          const lastContentPart = winMessage.slice(
            winMessage.indexOf("{winners}") + 9,
          );
          if (lastContentPart.length) {
            channel.send({
              content: lastContentPart,
              components:
                this.messages.winMessage.embed &&
                typeof this.messages.winMessage.embed === "object"
                  ? null
                  : components,
              allowedMentions: this.allowedMentions,
            });
        ...
        if (
          ...
          const embedDescription =
            embed.data.description?.replace("{winners}", formattedWinners) ??
            "";
          ...
          if (embedDescription.length <= 4096) {
            ...
            const firstEmbed = new Discord.EmbedBuilder(embed).setDescription(
              embed.data.description.slice(
                0,
                embed.data.description.indexOf("{winners}"),
              ) || null,
            );
            if (Discord.embedLength(firstEmbed.data)) {
              channel.send({
                ...
                reply: {
                  messageReference:
                    !(message?.length > 2000) &&
                    typeof this.messages.winMessage.replyToGiveaway ===
                      "boolean"
                      ? this.messageId
                      : undefined,
                  failIfNotExists: false,
            ...
            const tempEmbed = new Discord.EmbedBuilder().setColor(
            ...
            while (formattedWinners.length >= 4096) {
              await channel.send({
                embeds: [
                  tempEmbed.setDescription(
                    formattedWinners.slice(
                      0,
                      formattedWinners.lastIndexOf(",", 4095),
                    ) + ",",
                  ),
                ],
                allowedMentions: this.allowedMentions,
              ...
              formattedWinners = formattedWinners.slice(
                formattedWinners.slice(
                  0,
                  formattedWinners.lastIndexOf(",", 4095) + 2,
                ).length,
              );
            }
            channel.send({
            ...
            const lastEmbed = tempEmbed.setDescription(
              embed.data.description.slice(
                embed.data.description.indexOf("{winners}") + 9,
              ) || null,
            );
        ...
        const message = this.fillInString(
          noWinnerMessage?.content || noWinnerMessage,
        );
        ...
        if (message || embed) {
          channel.send({
            ...
            reply: {
              messageReference:
                typeof noWinnerMessage?.replyToGiveaway === "boolean"
                  ? this.messageId
                  : undefined,
              failIfNotExists: false,
            },
  ...
  reroll(options = {}) {
    return new Promise(async (resolve, reject) => {
      if (!this.ended)
        return reject(
          "Giveaway with message Id " + this.messageId + " is not ended.",
        );
      this.message ??= await this.fetchMessage().catch(() => {});
      if (!this.message)
        return reject(
      ...
      if (winners.length > 0) {
        ...
        let formattedWinners = winners.map((w) => `<@${w.id}>`).join(", ");
        const congratMessage = this.fillInString(
          options.messages.congrat.content || options.messages.congrat,
        );
        const message = congratMessage?.replace("{winners}", formattedWinners);
        const components = this.fillInComponents(
          options.messages.congrat.components,
        );

        if (message?.length > 2000) {
          const firstContentPart = congratMessage.slice(
            0,
            congratMessage.indexOf("{winners}"),
          );
          if (firstContentPart.length) {
            channel.send({
              content: firstContentPart,
          ...
          const lastContentPart = congratMessage.slice(
            congratMessage.indexOf("{winners}") + 9,
          );
          if (lastContentPart.length) {
            channel.send({
              content: lastContentPart,
              components:
                options.messages.congrat.embed &&
                typeof options.messages.congrat.embed === "object"
                  ? null
                  : components,
              allowedMentions: this.allowedMentions,
            });
            ...
            if (Discord.embedLength(firstEmbed.toJSON())) {
              channel.send({
                ...
                reply: {
                  messageReference:
                    !(message?.length > 2000) &&
                    typeof options.messages.congrat.replyToGiveaway ===
                      "boolean"
                      ? this.messageId
                      : undefined,
                  failIfNotExists: false,
        ...
        if (options.messages.replyWhenNoWinner !== false) {
          ...
          channel.send({
            content: this.fillInString(
              options.messages.error.content || options.messages.error,
            ),
            embeds: embed ? [embed] : null,
            components: this.fillInComponents(
              options.messages.error.components,
            ),
            allowedMentions: this.allowedMentions,
            reply: {
              messageReference:
  ...
  pause(options = {}) {
    return new Promise(async (resolve, reject) => {
      ...
      if (this.pauseOptions.isPaused) {
        return reject(
          "Giveaway with message Id " + this.messageId + " is already paused.",
        );
      }
  ...
  unpause() {
    return new Promise(async (resolve, reject) => {
      ...
      if (!this.pauseOptions.isPaused) {
        return reject(
          "Giveaway with message Id " + this.messageId + " is not paused.",
        );
      }
