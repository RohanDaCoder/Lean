...
class GiveawaysManager extends EventEmitter {
  ...
  generateMainEmbed(giveaway, lastChanceEnabled = false) {
    const embed = new Discord.EmbedBuilder()
      .setTitle(
        typeof giveaway.messages.title === "string"
      ...
      .setFooter({
        text:
          ...
          (typeof giveaway.messages.embedFooter === "string"
  ...
  generateEndEmbed(giveaway, winners) {
    ...
    const strings = {
      ...
      title:
        giveaway.fillInString(giveaway.messages.title) ??
        giveaway.fillInString(giveaway.prize),
    };
  ...
  generateNoValidParticipantsEndEmbed(giveaway) {
    const embed = new Discord.EmbedBuilder()
      ...
      .setColor(giveaway.embedColorEnd)
      .setFooter({
        text: giveaway.messages.endedAt,
        iconURL: giveaway.messages.embedFooter.iconURL,
      })
      .setDescription(
  ...
  start(channel, options) {
    return new Promise(async (resolve, reject) => {
      ...
      if (!channel?.id || !channel.isTextBased()) {
        return reject(
          `channel is not a valid text based channel. (val=${channel})`,
        );
      }
      ...
      if (
        typeof options.prize !== "string" ||
        (options.prize = options.prize.trim()).length > 256
      ) {
        return reject(
          `options.prize is not a string or longer than 256 characters. (val=${options.prize})`,
        );
      }
      ...
      if (options.isDrop && typeof options.isDrop !== "boolean") {
        return reject(
          `options.isDrop is not a boolean. (val=${options.isDrop})`,
        );
      }
      if (
        !options.isDrop &&
        (!Number.isFinite(options.duration) || options.duration < 1)
      ) {
        return reject(
          `options.duration is not a positive number. (val=${options.duration})`,
        );
      }
      ...
      const giveaway = new Giveaway(this, {
        ...
        messages:
          options.messages && typeof options.messages === "object"
            ...
            : GiveawayMessages,
        thumbnail:
          typeof options.thumbnail === "string" ? options.thumbnail : undefined,
        image: typeof options.image === "string" ? options.image : undefined,
        reaction: Discord.resolvePartialEmoji(options.reaction)
          ? options.reaction
          : undefined,
        botsCanWin:
          typeof options.botsCanWin === "boolean"
            ? options.botsCanWin
            : undefined,
        exemptPermissions: Array.isArray(options.exemptPermissions)
          ...
          : undefined,
        exemptMembers:
          typeof options.exemptMembers === "function"
            ? options.exemptMembers
            : undefined,
        bonusEntries:
          Array.isArray(options.bonusEntries) && !options.isDrop
            ? options.bonusEntries.filter((elem) => typeof elem === "object")
            : undefined,
        embedColor: validateEmbedColor(options.embedColor)
          ? options.embedColor
          : undefined,
        embedColorEnd: validateEmbedColor(options.embedColorEnd)
          ? options.embedColorEnd
          : undefined,
        extraData: options.extraData,
        lastChance:
          options.lastChance &&
          typeof options.lastChance === "object" &&
          !options.isDrop
            ? options.lastChance
            : undefined,
        pauseOptions:
          options.pauseOptions &&
          typeof options.pauseOptions === "object" &&
          !options.isDrop
            ? options.pauseOptions
            : undefined,
        allowedMentions:
    ...
    this.giveaways.forEach(async (giveaway) => {
      ...
      if (giveaway.ended) {
        if (
          ...
          this.giveaways = this.giveaways.filter(
            (g) => g.messageId !== giveaway.messageId,
          );
      ...
      if (giveaway.pauseOptions.isPaused) {
        if (
          ...
          giveaway.options.pauseOptions.durationAfterPause =
      ...
      if (!giveaway.message.embeds[0])
        await giveaway.message.suppressEmbeds(false).catch(() => {});
      ...
      const needUpdate =
        ...
        giveaway.message.content !==
          giveaway.fillInString(giveaway.messages.giveaway);
  ...
  async _handleRawPacket(packet) {
    ...
    if (!giveaway || (giveaway.ended && packet.t === "MESSAGE_REACTION_REMOVE"))
      return;
  ...
  async _init() {
    ...
    if (this.client.shard && this.client.guilds.cache.size) {
      ...
      rawGiveaways = rawGiveaways.filter(
        (g) =>
          shardId ===
          Discord.ShardClientUtil.shardIdForGuildId(
            g.guildId,
            this.client.shard.count,
          ),
      );
    ...
    if (Number.isFinite(this.options.endedGiveawaysLifetime)) {
      const endedGiveaways = this.giveaways.filter(
        (g) =>
          g.ended &&
          g.endAt + this.options.endedGiveawaysLifetime <= Date.now(),
      );
      this.giveaways = this.giveaways.filter(
        (g) =>
          !endedGiveaways
