...
module.exports = {
  ...
  run: async ({ client, interaction }) => {
    ...
    const senderDbPath = path.join(__dirname, `../../Database/${senderId}.json`);
    const senderProfile = new db(senderDbPath);
    const senderWallet = senderProfile.get("wallet") || 0;
    if (senderWallet < amount) {
      await interaction.editReply("You don't have enough coins in your wallet to make this transfer.");
      return;
    }

    const receiverDbPath = path.join(__dirname, `../../Database/${receiverId}.json`);
    ...
    await interaction.editReply(`You have successfully transferred ${amount} coins to ${receiverUsername}.`);
  }
};